...
ofl_msg_print_stats_reply(struct ofl_msg_stats_reply_header *msg, FILE *stream, struct ofl_exp *exp, char *errbuf) {
    if (msg->type == OFPST_EXPERIMENTER) {
        if (exp != NULL && exp->stats != NULL && exp->stats->reply_to_string != NULL) {
            ...
            if (errbuf != NULL) {
                snprintf(errbuf, OFL_ERRBUF_SIZE, "Trying to print EXPERIMENTER stats reply, but no callback was given.");
            }
            return -1;
        }
    }
...
ofl_msg_print(FILE *stream, struct ofl_msg_header *msg, struct ofl_exp *exp, char *errbuf) {
    ...
    switch (msg->type) {
        case OFPT_HELLO: { return 0; }
        case OFPT_ERROR: { ofl_msg_print_error((struct ofl_msg_error *)msg, stream); return 0; }
        case OFPT_ECHO_REQUEST:
        case OFPT_ECHO_REPLY: { ofl_msg_print_echo((struct ofl_msg_echo *)msg, stream); return 0; }
        ...
        case OFPT_FEATURES_REQUEST: { return 0; }
        case OFPT_FEATURES_REPLY: { ofl_msg_print_features_reply((struct ofl_msg_features_reply *)msg, stream); return 0; }
        case OFPT_GET_CONFIG_REQUEST: { return 0; }
        case OFPT_GET_CONFIG_REPLY: { ofl_msg_print_get_config_reply((struct ofl_msg_get_config_reply *)msg, stream); return 0; }
        case OFPT_SET_CONFIG: { ofl_msg_print_set_config((struct ofl_msg_set_config *)msg, stream); return 0; }

        /* Asynchronous messages. */
        case OFPT_PACKET_IN: { ofl_msg_print_packet_in((struct ofl_msg_packet_in *)msg, stream); return 0; }
        case OFPT_FLOW_REMOVED: { ofl_msg_print_flow_removed((struct ofl_msg_flow_removed *)msg, stream, exp); return 0; }
        case OFPT_PORT_STATUS: { ofl_msg_print_port_status((struct ofl_msg_port_status *)msg, stream); return 0; }

        /* Controller command messages. */
        case OFPT_PACKET_OUT: { ofl_msg_print_packet_out((struct ofl_msg_packet_out *)msg, stream, exp); return 0; }
        case OFPT_FLOW_MOD: { ofl_msg_print_flow_mod((struct ofl_msg_flow_mod *)msg, stream, exp); return 0; }
        ...
        /* Barrier messages. */
        case OFPT_BARRIER_REQUEST: { return 0; }
        case OFPT_BARRIER_REPLY: { return 0; }
