    ...
    switch(inst->type) {
        ...
        case (OFPIT_WRITE_METADATA): {
            ...
            fprintf(stream, "{meta=\"0x%"PRIx64"\", mask=\"0x%"PRIx64"\"}",
                          ntoh64(i->metadata), ntoh64(i->metadata_mask));
...
ofl_structs_match_print(FILE *stream, struct ofl_match_header *match, struct ofl_exp *exp) {
    ...
    switch (match->type) {
        case (OFPMT_STANDARD): {
            ...
            fprintf(stream, "std{wilds=\"%s%s%s%s%s%s%s%s%s%s\"",
                ((m->wildcards & OFPFW_IN_PORT) == 0) ? "" : "inport,",
                ((m->wildcards & OFPFW_DL_VLAN) == 0) ? "" : "vid,",
                ((m->wildcards & OFPFW_DL_VLAN_PCP) == 0) ? "" : "vpcp,",
                ((m->wildcards & OFPFW_DL_TYPE) == 0) ? "" : "dltype,",
                ((m->wildcards & OFPFW_NW_TOS) == 0) ? "" : "nwtos,",
                ((m->wildcards & OFPFW_NW_PROTO) == 0) ? "" : "nwprt,",
                ((m->wildcards & OFPFW_TP_SRC) == 0) ? "" : "tpsrc,",
                ((m->wildcards & OFPFW_TP_DST) == 0) ? "" : "tpdst,",
                ((m->wildcards & OFPFW_MPLS_LABEL) == 0) ? "" : "mplslbl,",
                ((m->wildcards & OFPFW_MPLS_TC) == 0) ? "" : "mplstc,");
            ...
            if (((m->wildcards & OFPFW_DL_VLAN_PCP) == 0) && (ntohs(m->dl_vlan) != OFPVID_NONE)) {
                fprintf(stream, ", vlanpcp=\"%u\"", m->dl_vlan_pcp);
            }
            if ((m->wildcards & OFPFW_DL_TYPE) == 0) {
                fprintf(stream, ", dltype=\"0x%"PRIx16"\"", ntohs(m->dl_type));
            }
            ...
            if ((m->wildcards & OFPFW_TP_SRC) == 0) {
                fprintf(stream, ", tpsrc=\"%u\"", ntohs(m->tp_src));
            }
            if ((m->wildcards & OFPFW_TP_DST) == 0) {
                fprintf(stream, ", tpdst=\"%u\"", ntohs(m->tp_dst));
            }
            if (((m->dl_type == htons(ETH_TYPE_MPLS) || m->dl_type == htons(ETH_TYPE_MPLS))) && ((m->wildcards & OFPFW_MPLS_LABEL) == 0)) {
                fprintf(stream, ", mplslbl=\"%u\"", ntohl(m->mpls_label));
            }
            if (((m->dl_type == htons(ETH_TYPE_MPLS) || m->dl_type == htons(ETH_TYPE_MPLS))) && ((m->wildcards & OFPFW_MPLS_TC) == 0)) {
                fprintf(stream, ", mplstc=\"%u\"", m->mpls_tc);
            }
            if (memcmp(&m->metadata_mask, mask_all, 8) == 0) {
                fprintf(stream, ", metam=\"all\"");
            } else {
                fprintf(stream, ", meta=\"0x%"PRIx64"\"", ntoh64(m->metadata));
                if (memcmp(&m->metadata_mask, mask_none, 8) != 0) {
                    fprintf(stream, ", metam=\"0x%"PRIx64"\"", ntoh64(m->metadata_mask));
                }
            }
            fprintf(stream, "}");
